...
public class CheckBoxActionComponent  extends ActionComponent implements Reflection{
...
private static final Class<?> DEFAULT_EVENT_TYPE = ClickHandler.class;
	...
	public void init(ActionBinding actionBinding) {
		...
		checkBox=(CheckBox)actionBinding.getComponent();
	}
	...
	public void bind() {
		...
		checkBox.addDoubleClickHandler(doubleClickHandler);	
		}
		...
		focusHandler=new FocusHandler() {
			...
			checkBox.addFocusHandler(focusHandler);
		}
		...
		if(eventType==GestureChangeHandler.class){
				...
				checkBox.addGestureChangeHandler(gestureChangeHandler);
		...
		if(eventType==KeyDownHandler.class){
			...
			checkBox.addKeyDownHandler(keyDownHandler);
		}
        ...
        if(eventType==KeyPressHandler.class){
			...
			checkBox.addKeyPressHandler(keyPressHandler);
		}
        ...
        if(eventType==KeyUpHandler.class){
			...
			checkBox.addKeyUpHandler(keyUpHandler);
		}
        ...
        if(eventType==MouseDownHandler.class){
			...
			checkBox.addMouseDownHandler(mouseDownHandler);
		}
		...
		mouseMoveHandler= new MouseMoveHandler() {
			...
			checkBox.addMouseMoveHandler(mouseMoveHandler);
		}
        ...
        if(eventType==KeyUpHandler.class){
	       mouseOutHandler= new MouseOutHandler() {
			...
			checkBox.addMouseOutHandler(mouseOutHandler);
		}
        ...
        if(eventType==MouseOverHandler.class){
			...
			checkBox.addMouseOverHandler(mouseOverHandler);
		}
		...
		mouseUpHandler= new MouseUpHandler() {
			...
			checkBox.addMouseUpHandler(mouseUpHandler);
		}
		...
		mouseWheelHandler= new MouseWheelHandler() {
			...
			checkBox.addMouseWheelHandler(mouseWheelHandler);
		}
        ...
        if(eventType==KeyUpHandler.class){
		   ...
		   touchCancelHandler = new TouchCancelHandler() {
			...
			checkBox.addTouchCancelHandler(touchCancelHandler);
		}
        ...
        if(eventType==TouchEndHandler.class){
			...
			checkBox.addTouchEndHandler(touchEndHandler);
		}
        ...
        if(eventType==KeyUpHandler.class){
			...
			checkBox.addTouchMoveHandler(touchMoveHandler);
		}
		...
		touchStartHandler= new TouchStartHandler() {
        	...
        	checkBox.addTouchStartHandler(touchStartHandler);
		}
